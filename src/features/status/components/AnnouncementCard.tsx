...
const ANNOUNCEMENT_CARD_CLASS: Record<CardVariant, string> = {
  web: "flex h-41.25 w-94.5 flex-col rounded-2xl border border-admin-outline/10 bg-admin-outline/10 px-7 py-6 shadow-xl",
  mobile:
    "flex h-37.75 w-87.5 items-center justify-center rounded-2xl border border-admin-outline/10 bg-admin-box shadow-xl",
};
...
function AnnouncementCard({ announcement, variant = "web" }: AnnouncementCardProps) {
  const isMobile = variant === "mobile";
  const statusTextClass =
    isMobile ? "text-[11px]" : "text-xs";
  const titleTextClass =
    isMobile ? "text-sm" : "text-sm";
  const periodTextClass =
  ...
  return (
    <article className={ANNOUNCEMENT_CARD_CLASS[variant]}>
      <div className={contentWrapperClass}>
        ...
        {/* 모집 기간 */}
        <div
          className={`${periodSpacingClass} ${periodTextClass} font-normal text-admin-sub`}
        >
          {announcement.period}
        </div>
        ...
        {/* 제출/임시저장 통계 */}
        <div
          className={`${statsSpacingClass} ${statsTextClass} font-normal text-admin-white/70`}
        >
          {announcement.stats}
        </div>
      </div>
